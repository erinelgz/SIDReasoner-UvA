...
#SBATCH --gpus=4
#SBATCH --job-name=sid-yelp-stage1-sft
#SBATCH --ntasks=1
...
OUTPUT_DIR="${OUTPUT_DIR:-./output_dir/${CATEGORY}_stage1_sft_Qwen3-1.7B}"
RUN_NAME="${RUN_NAME:-${CATEGORY}_stage1_sft_Qwen3-1.7B}"
...
CUDA_VISIBLE_DEVICES="${CUDA_DEVICES}" ${TORCHRUN_CMD} --nproc_per_node "${NPROC_PER_NODE}" \
    ...
    --llm_generated_data_path "./data/Yelp/index/Yelp_Restaurants.item_enhanced_v2.json" \
    --llm_generated_sequence_path "./data/Yelp/index/Yelp_Restaurants.integrated_narrative.csv" \
    --general_reasoning_path "./data/Yelp/general/sampled_data.arrow" \
    --mask_assistant True \
    "$@"
} > "${LOG_FILE}" 2>&1
